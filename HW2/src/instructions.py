...
class Instructions():
    ...
    def __init__(self, instructions: list[str]):
        ...
        self.bbs = [0] # basic block end pc indices # after init bbs is either [0, final_pc] or [0, loop_jump, loop_pc, final_pc]
        ...
        for i in instructions:
            ...
            if instr["type"] ==  "branch": 
                assert instr["branch"] < pc, "Branch target must be a previous instruction."
                self.bbs.extend([instr["branch"], pc]) # (label, pc)
                
            self.instructions.append(instr) # opcode, type, dest, ops, imm, branch, lc_ec, pred_reg
            pc += 1
    ...
    def separate_rename_ops(self, info: dict) -> dict:
        ...
        if info["opcode"] in ("add", "addi", "sub", "mov"):
            ...
            imm, mem_reg = (non_regs[0].split("(")[0], non_regs[0].split("(")[1][:-1][1:]) if "(" in non_regs[0] else (None, None)
            
            assert imm is not None, "Memory address must be provided for load/store instructions."
            assert mem_reg is not None, "Memory address must be provided for load/store instructions."
...
if __name__ == "__main__":
    import json
    # testing 
    with open("/home/ygifoom/epfl/aa/CS470-Homeworks/HW2/given_tests/09/input.json", "r") as f:
