...
parser.add_argument(
    "result", type=argparse.FileType("w"), help="The cycle-accurate simulation result."
)
parser.add_argument(
    "--memory",
    type=argparse.FileType("r"),
...
class VLIW470:
    ...
    ALU1Pipe = {"predicate": False, "opcode": "", "targetReg": 0, "value": 0}
    ...
    def updateRegister(self, name: str, value: int):
        if name in self._debug_currentCycleUpdate:
            print(
                "Warning: Multiple instructions are updating the register {}.".format(
                    name
                )
            )
        else:
    ...
    def renameRegister(self, idx: int) -> int:
        ...
        assert idx >= 0 and idx < 96, (
            "Trying to rename a register out of the specific range."
        )
    ...
    def decodeALUInstruction(self, i: str) -> dict:
        ...
        assert opcode in ["add", "addi", "sub", "mov", "nop"], (
            "Undefined instruction: {}".format(i)
        )
        ...
        if opcode in ["add", "addi", "sub"]:
            ...
            assert ops[0].startswith("x"), (
                "Cannot determine the destination: {}".format(i)
            )
            dest = self.renameRegister(int(ops[0][1:]))

            assert ops[1].startswith("x"), "Cannot determine the source reg: {}".format(
                i
            )
            ...
            if opcode == "addi":
                ...
                assert ops[2].startswith("x"), (
                    "Cannot determine the 2nd source reg: {}".format(i)
                )
            ...
            if opcode == "add":
                result = (
                    self.PhysicalRegisterFile[src1] + self.PhysicalRegisterFile[src2]
                )
            elif opcode == "addi":
            ...
            if ops[0].startswith("p"):
                ...
                if ops[0].upper() == "RBB":
                    assert value < 64, (
                        "The maximum value of RBB is 63. The value you provide causes overflow."
                    )
                return {
                    "predicate": predication,
                    "opcode": "update{}".format(
                        dest
                    ),  # alu, updateLC, updateEC, updateRBB, updatePredicate
                    "targetReg": 0,
                    "value": value,
                }
    ...
    def decodeMultiplierInstruction(self, i: str) -> dict:
        ...
        if opcode == "mulu":
            assert (
                ops[0].startswith("x")
                and ops[1].startswith("x")
                and ops[2].startswith("x")
            ), "Undefined instruction: {}".format(i)
            dest = self.renameRegister(int(ops[0][1:]))
            src1 = self.renameRegister(int(ops[1][1:]))
            src2 = self.renameRegister(int(ops[2][1:]))
    ...
    def decodeLoadStoreInstruction(self, i: str) -> dict:
        ...
        # ops[0]: xNN
        # ops[1]: imm(xMM)
        assert ops[0].startswith("x") and "(" in ops[1] and ")" in ops[1], (
            "Undefined instruction: {}".format(i)
        )
    ...
    def decodeBrancInstruction(self, i: str) -> dict:
        ...
        assert opcode in ["loop", "loop.pip", "nop"], (
            "Undefined instruction: {}".format(i)
        )
    ...
    def tick(self):
        ...
        for aluPipe in [self.ALU0Pipe, self.ALU1Pipe]:
            if aluPipe["predicate"]:
                ...
                if aluPipe["opcode"] == "alu":
                    ...
                    assert False, "Wrong opcode is provided: {}".format(
                        aluPipe["opcode"]
                    )
        ...
        if self.MemoryPipe["predicate"]:
            if self.MemoryPipe["opcode"] == "load":
                ...
                dataMemory.write(self.MemoryPipe["address"], self.MemoryPipe["data"])
            else:
